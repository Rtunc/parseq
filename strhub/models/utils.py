...
def load_from_checkpoint(checkpoint_path: str, **kwargs):
    if checkpoint_path.startswith('pretrained='):
        ...
        print("loading from your pretrained model!")
        if "parseq-tiny" in checkpoint_path:
            experiment = "parseq-tiny"
        else:
            print("please rename checkpoint name so that code get out the experiment file")
        try:
            config = _get_config(experiment, **kwargs)
        except FileNotFoundError:
            raise InvalidModelError(f"No configuration found for '{experiment}'") from None
